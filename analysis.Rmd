...
---
# Importing the concatenated data report of Johns Hopkins University Novel Corona Virus data sets
...
confirmed_cases_worldwide <- read_csv(
    "./datasets/confirmed_cases_worldwide.csv", 
    col_types = list(
...
# Reading source data set and filtering tidy data set
confirmed_cases_worldwide_new <- coronavirus %>% 
  group_by(date, cases) %>%
  filter(type == "confirmed") %>%
  mutate(cum_cases = cumsum(cases)) %>%
  summarize(cum_case = max(cum_cases))
  arrange(date)

glimpse(confirmed_cases_worldwide_new)
...
ggplot(confirmed_cases_worldwide_new, aes(date, cum_cases)) + 
       geom_line() + 
       ylab("Cumulative confirmed cases")
```
...
confirmed_cases_china_vs_world <- read_csv("./datasets/confirmed_cases_china_vs_world.csv", 
                                           col_types= list(is_china = col_character(),
                                                        date = col_date(format = ""),
                                                        cases = col_double(),
                                                        cum_cases = col_double()))
...
# Read in source and generate new tidy data set
confirmed_cases_china_vs_world_new <- coronavirus %>%
  mutate(is_china = replace(Country.Region, Country.Region != "China", "Not China" )) %>% 
  as.data.frame() %>%
...
# Draw a line plot of cumulative cases vs. date, grouped and colored by is_china
# Define aesthetics within the line geom
plt_cum_confirmed_cases_china_vs_world_new <- ggplot(confirmed_cases_china_vs_world_new) +
  geom_line(aes(date, cum_cases, group = is_china, color = is_china)) +
  ylab("Cumulative confirmed cases")

# See the plot
plt_cum_confirmed_cases_china_vs_world_new
```
...
ggplot(china_after_feb15, aes(cum_cases, date)) +
  geom_line() +
...
## 9. Plotting hardest hit countries

Even though the outbreak was first identified in China, four of the listed countries (France, Germany, Italy, and Spain) are in Europe and share borders. To get more context, we can plot these countries' confirmed cases over time.

```{r}
# Run this to get the data for the top 7 countries
confirmed_cases_top07_outside_china <- read_csv(
...
#
glimpse(confirmed_cases_top07_outside_china)
```
...
top_7_countries <- top_countries_by_total_cases$country[1:7]

top_7 <- coronavirus %>% 
  group_by(Country.Region, date, type) %>%
  select(date, country = Country.Region, type, cases) %>%
  filter(type == "confirmed", country %in% top_7_countries) %>%
  summarize(cum_cases = max(sum(cases))) %>%
  arrange(date)

ggplot(top_7, aes(date, cum_cases, color = country, group = country), fill = "transparent") +
    geom_line() +
    ylab("Cumulative confirmed cases")
```
