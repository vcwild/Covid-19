...
confirmed_cases_worldwide_new <- coronavirus %>% group_by(date, type) %>%
...
# Draw a line plot of cumulative cases vs. date, grouped and colored by is_china
# Define aesthetics within the line geom
plt_cum_confirmed_cases_china_vs_world <- ggplot(confirmed_cases_china_vs_world_new) +
  geom_line(aes(date, cum_cases, group = is_china, color = is_china)) +
  ylab("Cumulative confirmed cases")
...
```{r}
confirmed_cases_by_country <- read_csv(
    "datasets/confirmed_cases_by_country.csv",  
    col_types = list(
...
ggplot(confirmed_cases_top7_outside_china, aes(date, cum_cases, color = country, group = country), fill = "transparent") +
    geom_line() +
    ylab("Cumulative confirmed cases")
```
